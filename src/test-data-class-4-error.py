...
# dataclassでデフォルト値を使うと、それ以降の位置にある項目もデフォルト値をセットせねばならない。このルールはメソッドの引数と同じ。位置引数、オプション引数の関係と同じ。
# この制約があるため、DBレコードのdefault制約と同じことを再現できない。
...
class TestDataClass(unittest.TestCase):
    ...
    def test_annotations(self):
        self.assertEqual(['id', 'name', 'birth', 'value'], list(Record.__annotations__.keys()))
        self.assertEqual([int, str, datetime, Decimal], list(Record.__annotations__.values()))
    def test_dataclass_fields(self):
        self.assertEqual(['id', 'name', 'birth', 'value'], list(Record.__dataclass_fields__.keys()))
